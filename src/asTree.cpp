...
std::string tokenTypeToString(TokenType type) {
    switch (type) {
        ...
        case TokenType::AtAt: return "AtAt";
...
std::string NumberLiteralNode::toStr(int indent) const {
    if (std::holds_alternative<int>(token.value)) {
        ...
        return std::to_string(indent) + "aa";
    else if (std::holds_alternative<std::monostate>(token.value))
        return "b";
    return "";
}

std::string StringLiteralNode::toStr(int indent) const {
    return std::string(indent, ' ') + "\"" + getValue() + "\"";
}
...
std::string BinaryOpNode::toStr(int indent) const {
    ...
    result += left->toStr(indent + 1) + " " + tokenTypeToString(opToken.type) + " " + right->toStr(indent + 1);
    result += ")";
    return result;
}

std::string TypeCastNode::toStr(int indent) const {
    return std::string(indent, ' ') + "(" + expression->toStr(indent + 1) + " as " + tokenTypeToString(typeToken.type) + ")";
}

std::string FunctionCallNode::toStr(int indent) const {
    std::string out = std::string(indent, ' ') + callee->toStr(indent + 1) + "(";
...
std::string DeclarationNode::toStr(int indent) const {
    std::string result = std::string(indent, ' ') + tokenTypeToString(typeModifierToken.type) + " " + getIdentifierName();
    if (initializer) {
...
std::string FunctionDeclarationNode::toStr(int indent) const {
    ...
    for (size_t i = 0; i < params.size(); ++i) {
        result += tokenTypeToString(params[i].modifierToken.type) + " " + params[i].id.getValue<std::string>();
        if (i < params.size() - 1) {
            result += ", ";
        }
    }
...
std::string FunctionLiteralNode::toStr(int indent) const {
    std::string result = std::string(indent, ' ') + "fun " + funToken.getValue<std::string>() + "(";
    for (size_t i = 0; i < parameters.size(); ++i) {
        result += tokenTypeToString(parameters[i].modifierToken.type) + " " + parameters[i].id.getValue<std::string>();
        if (i < parameters.size() - 1) {
            result += ", ";
        }
    }
    result += ") " + body->toStr(indent + 1);
    return result;
}

std::string IfStatementNode::toStr(int indent) const {
    std::string result = std::string(indent, ' ') + "if (" + condition->toStr(indent + 1) + ") " + thenBlock->toStr(indent + 1);
    if (elseBlock) {
        result += " else " + elseBlock->toStr(indent + 1);
    }
    return result;
}
...
std::string AssignNode::toStr(int indent) const {
    return std::string(indent, ' ') + identifierToken.getValue<std::string>() + " = " + expression->toStr(indent + 1) + ";";
}

std::string WhileStatementNode::toStr(int indent) const {
    return std::string(indent, ' ') + "while (" + condition->toStr(indent + 1) + ") " + body->toStr(indent + 1);
}
