#pragma once
 ...
 public:
     ...
     BinaryOpNode(std::unique_ptr<ExpressionNode> left, Token opToken, std::unique_ptr<ExpressionNode> right)
 ...
 public:
     ...
     UnaryOpNode(Token opToken, std::unique_ptr<ExpressionNode> operand)
         : opToken(opToken), operand(std::move(operand)) {}
     Position getStartPosition() const override { return opToken.startPosition; }
 };
 ...
 public:
     std::vector<std::pair<Token, Token>> parameters; // <type, id>
     std::unique_ptr<StatementBlockNode> body;
     Token funToken;
     Token lParenToken;
     Token rParenToken;

     FunctionLiteralNode(Token funToken, Token lParenToken, std::vector<std::pair<Token, Token>> parameters, Token rParenToken, std::unique_ptr<StatementBlockNode> body)
         : funToken(funToken), lParenToken(lParenToken), parameters(std::move(parameters)), rParenToken(rParenToken), body(std::move(body)) {}
 ...
 public:
     ...
     StatementBlockNode(Token lBracketToken, std::vector<std::unique_ptr<StatementNode>> statements, Token rBracketToken)
         : statements(std::move(statements)), lBracketToken(lBracketToken), rBracketToken(rBracketToken) {}
 ...
 public:
     ...
     IfStatementNode(Token ifToken, Token lParenToken, std::unique_ptr<ExpressionNode> condition, Token rParenToken, std::unique_ptr<StatementBlockNode> thenBlock, std::unique_ptr<StatementBlockNode> elseBlock = nullptr, Token elseToken = Token(TokenType::Unknown, Position()))
         : condition(std::move(condition)), thenBlock(std::move(thenBlock)), elseBlock(std::move(elseBlock)), ifToken(ifToken), lParenToken(lParenToken), rParenToken(rParenToken), elseToken(elseToken) {}
 ...
 public:
     ...
     DeclarationNode(Token typeModifierToken, Token identifierToken, std::unique_ptr<ExpressionNode> initializer = nullptr, Token assignToken = Token(TokenType::Unknown, Position()))
         : typeModifierToken(typeModifierToken), identifierToken(identifierToken), initializer(std::move(initializer)), assignToken(assignToken) {}
 ...
 public:
     ...
     AssignNode(Token identifierToken, Token assignToken, std::unique_ptr<ExpressionNode> expression)
         : identifierToken(identifierToken), assignToken(assignToken), expression(std::move(expression)) {}
 ...
 public:
     ...
     WhileStatementNode(Token whileToken, Token lParenToken, std::unique_ptr<ExpressionNode> condition, Token rParenToken, std::unique_ptr<StatementBlockNode> body)
         : condition(std::move(condition)), body(std::move(body)), whileToken(whileToken), lParenToken(lParenToken), rParenToken(rParenToken) {}
 ...
 public:
     ...
     ProgramNode(std::vector<std::unique_ptr<AstNode>> declarations) : declarations(std::move(declarations)) {}
     Position getStartPosition() const override { return declarations.empty() ? Position() : declarations.front()->getStartPosition(); }
 };
